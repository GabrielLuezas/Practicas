...
import pymongo
from flask_jwt_extended import create_access_token, get_jwt_identity, jwt_required, JWTManager, verify_jwt_in_request, set_access_cookies
import datetime 
    ...
    if request.method == 'POST':
        ...
        for verificacion in lista_usuarios:
            if verificacion["username"] == username and verificacion["password"] == password:
                ...
                return redirect(url_for('protegida', token=access_token))
            else:
...
def login2():
    ...
    for verificacion in lista_usuarios:
        if verificacion["username"] == username and verificacion["password"] == password:
            access_token = create_access_token(identity=username)
            return redirect(url_for('protegida', token=access_token), code=307)
        else:
            return jsonify({"msg": "El usuario o la contraseña son incorrectos"}), 401
